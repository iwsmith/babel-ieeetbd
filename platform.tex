...
With all of this in mind we propose online evaluation using long-term, implicit field studies focused on two outcomes: did a user view a paper's abstract and did a user download a paper.
...
Our design goals centered around providing low-latency recommendations suitable for client consumption from a web browser, while scaling horizontally to support many concurrent users. %TODO: Would be nice to have an actual number
...
Their output is then pushed to the recommendation cache, currently implemented in DynamoDB. 
