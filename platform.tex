...
Some have chosen to crawl publisher to create their own datasets, but this risks legal action over redistribution of copyrighted material, also limiting reproducibility.
...
Although techniques exist for evaluating recommenders in other domaisn with explicitly rated items (such as movies or products) there is no agreement on how recommenders for unrated implicit datasets should be evaluated.
...
A day later they will be able to compare the performance of their algorithm to other recommenders on live users.
We hope that this fast turn around will allow researchers to focus on improving the quality of their algorithms, not the details of acquiring datasets and tracking user behavior. %TODO: Add comment about our datasets here I think
...
Although this approach is valid for recommendations that are explicitly rated it fails for implicitly rated data.
...
Consider, for example, the MovieLens dataset, which contains 20 million ratings for 27,000 movies; nearly three orders of magnitude more user-item interactions than there are items.
...
There are also questions of bias and coverage; everyone has certain preferences, and in some fields it may be difficult to find a local expert.
...
Babel's feedback mechanism collects additional facets (some optional) which can be used to group or filter data, including: consumer (i.e. publisher/platform), session identifier, database (e.g. PLOS, arXiv, MAS), pseudonymous user identifier and time.
...
The public endpoint is what consumers will interact with, and is represented in figure~\ref{fig:arch} by 1.
This endpoint exposes a REST API (optionally over SSL), which, given a paper identifier and a publisher, will generate a list of recommendations with various recommendations algorithms, returned in JSON format. 
...
Our plan is to onboard several publishers in the coming months.
Once we have validated our design to deliver recommendations and record feedback we will being adding new recommenders.
Only the EigenFactor Recommends recommenders are implemented currently, but we will offer co-citation shortly.
You can try out the platform at our demonstration website: \url{babel.eigenfactor.org}.
